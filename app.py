...
def crear_zip_todos_expedientes(resultados):
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zipf:
        for r in resultados:
            if "error" in r:
                continue
            carpeta_nombre = limpiar_nombre(f"EXPEDIENTE_{r['folio']}_{r['factura']}")

            archivos = [
                (r.get("requisicion"), "REQUISICION"),
                (r.get("cotizacion"), "COTIZACION"),
                (r.get("xml"), "XML"),
            ]

            for ruta, _tipo in archivos:
                if ruta and os.path.exists(ruta):
                    zipf.write(ruta, os.path.join(carpeta_nombre, os.path.basename(ruta)))

            for pdf in r.get("pdfs", []):
                if pdf and os.path.exists(pdf):
                    zipf.write(pdf, os.path.join(carpeta_nombre, os.path.basename(pdf)))

            if not r.get("pdfs") and r.get("pdf") and os.path.exists(r["pdf"]):
                zipf.write(r["pdf"], os.path.join(carpeta_nombre, os.path.basename(r["pdf"])))
...
if pendientes:
    ...
    if facturas_validas:
        ...
        # Resumen rápido antes de generar: cuánto gastará cada área según la selección actual
        resumen_previo = {}
        for i, f in enumerate(facturas_validas):
            if not st.session_state.get(f"generar_factura_{i}", True):
                continue
            area_tmp = st.session_state.get(f"area_factura_{i}", "Administración")
            total_tmp = float(f.get("datos", {}).get("total", 0) or 0)
            resumen_previo[area_tmp] = resumen_previo.get(area_tmp, 0.0) + total_tmp

        if resumen_previo:
            st.markdown("### 💰 Resumen antes de generar")
            st.table([
                {"Área": area, "Total": f"${total:,.2f}"}
                for area, total in sorted(resumen_previo.items(), key=lambda x: x[1], reverse=True)
            ])
...
if resultados:
    ...
    if resultados_validos:
        zip_todos = crear_zip_todos_expedientes(resultados_validos)
        st.download_button(
            label="📦 Descargar TODOS los expedientes en un ZIP",
            data=zip_todos,
            file_name=f"TODOS_LOS_EXPEDIENTES_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip",
            mime="application/zip",
            key="zip_todos_expedientes"
        )
...
# =====================================================
st.markdown("---")
st.markdown("## 📊 Dashboard Ejecutivo DIF")
st.caption("Aquí puedes ver quién gastó más, cuánto gastó cada área y descargar el reporte del periodo.")
...
proveedor_mayor = next(iter(res_proveedor.keys())) if res_proveedor else "Sin datos"
...
c4.metric("Proveedor principal", proveedor_mayor)
...
if filtrados:
    ...
    # -----------------------------
    resumen_area = []
    for area, total in res_area.items():
        facturas_area = [r for r in filtrados if (r.get("area") or "Sin dato") == area]
        resumen_area.append({
            "Área": area,
            "Facturas": len(facturas_area),
            "Total gastado": round(total, 2)
        })

    st.subheader("🏢 Resumen por área")
    st.dataframe(resumen_area, use_container_width=True, hide_index=True)

    st.subheader("📈 Gráfica de gasto por área")
    ...
    # -----------------------------
    resumen_proveedor = []
    for proveedor, total in res_proveedor.items():
        facturas_proveedor = [r for r in filtrados if (r.get("proveedor") or "Sin dato") == proveedor]
        resumen_proveedor.append({
            "Proveedor": proveedor,
            "Facturas": len(facturas_proveedor),
            "Total gastado": round(total, 2)
        })
    ...
    st.subheader("📈 Gráfica de gasto por proveedor")
    ...
    with col_desc1:
        st.download_button(
            ...
            file_name=f"detalle_gastos_{fecha_inicio}_a_{fecha_fin}.csv",
            mime="text/csv"
        )

    resumen_area_csv = io.StringIO()
    writer_area = csv.DictWriter(resumen_area_csv, fieldnames=["Área", "Facturas", "Total gastado"])
    writer_area.writeheader()
    for r in resumen_area:
        writer_area.writerow(r)
    with col_desc2:
        st.download_button(
            "⬇️ Descargar resumen áreas",
            data=resumen_area_csv.getvalue().encode("utf-8-sig"),
            file_name=f"resumen_por_area_{fecha_inicio}_a_{fecha_fin}.csv",
            mime="text/csv"
        )

    resumen_proveedor_csv = io.StringIO()
    writer_prov = csv.DictWriter(resumen_proveedor_csv, fieldnames=["Proveedor", "Facturas", "Total gastado"])
    writer_prov.writeheader()
    for r in resumen_proveedor:
        writer_prov.writerow(r)
    with col_desc3:
        st.download_button(
            "⬇️ Descargar resumen proveedores",
            data=resumen_proveedor_csv.getvalue().encode("utf-8-sig"),
            file_name=f"resumen_por_proveedor_{fecha_inicio}_a_{fecha_fin}.csv",
            mime="text/csv"
        )
else:
...
# =====================================================
# REINICIO DE HISTORIAL DE PRUEBAS
# =====================================================
st.markdown("---")
st.markdown("## 🧪 Herramientas de prueba")
st.warning("Usa esta opción solamente si quieres borrar el historial de movimientos de prueba. No borra las plantillas ni los expedientes generados.")
confirmar_borrado = st.checkbox("Confirmo que quiero reiniciar el historial de gastos de prueba")
if st.button("🗑️ Reiniciar historial de pruebas"):
    if confirmar_borrado:
        try:
            ...
            inicializar_archivo_gastos()
            st.session_state["resultados_generados"] = []
            st.success("Historial de gastos reiniciado correctamente. Ya puedes empezar a registrar áreas reales.")
            st.rerun()
        except Exception as e:
            st.error(f"No se pudo reiniciar el historial: {e}")
    else:
        st.error("Primero marca la casilla de confirmación para evitar borrar datos por accidente.")
