...
st.markdown("""
<style>
/* ===== FONDO GENERAL TIPO DIF ===== */
.stApp {
    background:
        radial-gradient(circle at -3% 8%, rgba(8,123,117,0.30) 0%, rgba(8,123,117,0.16) 18%, transparent 32%),
        radial-gradient(circle at 106% 92%, rgba(233,78,27,0.42) 0%, rgba(233,78,27,0.24) 22%, transparent 38%),
        linear-gradient(135deg, #EEF8F5 0%, #FFF7E7 50%, #F8C2A5 100%);
    min-height: 100vh;
}

/* Oculta un poco el espacio superior de Streamlit */
.block-container {
    padding-top: 25px;
    max-width: 980px;
}
...
/* IMPORTANTE: el logo es JPG y trae fondo blanco.
   Esto mezcla el blanco del JPG con el fondo de color. */
.logo-card img {
